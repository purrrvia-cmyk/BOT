...
class ICTStrategy:
    ...
    def generate_signal(self, symbol, df, multi_tf_data=None):
        ...
        if unicorn:
            ...
            # ★ FIX: FVG kenar giriş (CE değil) — daha gerçekçi fill
            # CE (orta nokta) idealist, fiyat çoğu zaman FVG kenarından döner
            # LONG: FVG üst kenarı (fiyat inip bu kenardan döner)
            # SHORT: FVG alt kenarı (fiyat çıkıp bu kenardan döner)
            if bias == "LONG":
                entry = entry_fvg["high"]  # FVG üst kenar (discount giriş)
            else:
                entry = entry_fvg["low"]   # FVG alt kenar (premium giriş)
        ...
        # ★ FIX: Entry mesafe kontrolü — entry current_price'tan çok uzaksa sinyal at
        # 15m TF'de %1.5'ten uzak entry = piyasa geri gelmez, bekleme kaybı oluşur
        entry_distance_pct = abs(current_price - entry) / current_price
        if entry_distance_pct > 0.015:  # %1.5'ten uzak
            logger.info(f"❌ {symbol}: Entry çok uzak ({entry_distance_pct:.2%}) → sinyal atlandı")
            return None

        # ★ FIX: MARKET order için genişletilmiş koşullar
        # A+ tier (MSS onay) → fiyat FVG yakınındaysa (%0.5 içinde) MARKET gir
        # A tier → fiyat tam FVG'deyse MARKET, değilse LIMIT
        # Eski: Sadece price_at_fvg AND mss_confirmed → yarı MARKET
        near_fvg = entry_distance_pct <= 0.005  # %0.5 içinde
        if confirmation["mss_confirmed"] and (price_at_fvg or near_fvg):
            entry_mode = "MARKET"  # A+ tier, MSS onaylı + yakın → MARKET
            entry = current_price   # MARKET → güncel fiyattan gir
        elif price_at_fvg:
            entry_mode = "MARKET"  # Fiyat tam FVG'de → MARKET
            entry = current_price
        else:
            entry_mode = "LIMIT"   # Diğer durumlar → LIMIT (FVG edge'e emir)

        # ★ MARKET modunda entry değişti → SL/TP/RR yeniden doğrula
