...
class TradeManager:
    ...
    def _check_waiting_signal(self, signal, current_price, entry_price,
        ...
        # ★ FIX: ESKİMİŞ SİNYAL TESPİTİ — Fiyat zaten TP yönüne çok gitmişse iptal
        # Kullanıcı sorunu: Sinyal WAITING'de beklerken fiyat TP yönüne %2-5 gidiyor,
        # sonra geri dönüyor = entry yapıldığında trend zaten bitmiş.
        # Fiyat entry'den TP yönüne %1.5+ gittiyse → setup eskimiş, iptal et
        ...
        if take_profit and entry_price:
            if direction == "LONG" and current_price > entry_price:
                ...
                if tp_distance > 0 and price_moved / tp_distance > 0.40:
                    # Fiyat zaten TP mesafesinin %40'ını geçmiş → ters gelebilir
                    update_signal_status(signal_id, "CANCELLED", close_price=current_price, pnl_pct=0)
                    move_pct = (price_moved / entry_price) * 100
                    logger.info(
                        f"⏭️ ESKİMİŞ SİNYAL: #{signal_id} {symbol} LONG | "
                        f"Fiyat entry'den +{move_pct:.2f}% yukarıda (TP %{price_moved/tp_distance*100:.0f}) → iptal"
                    )
                    return {
                        "signal_id": signal_id, "symbol": symbol,
                        "direction": direction, "status": "CANCELLED",
                        "reason": f"Fiyat zaten TP yönüne gitmiş (+{move_pct:.2f}%)"
                    }
                ...
                if tp_distance > 0 and price_moved / tp_distance > 0.40:
                    ...
                    move_pct = (price_moved / entry_price) * 100
                    logger.info(
                        f"⏭️ ESKİMİŞ SİNYAL: #{signal_id} {symbol} SHORT | "
                        f"Fiyat entry'den -{move_pct:.2f}% aşağıda (TP %{price_moved/tp_distance*100:.0f}) → iptal"
                    )
                    return {
                        "signal_id": signal_id, "symbol": symbol,
                        "direction": direction, "status": "CANCELLED",
        ...
        entry_buffer = entry_price * 0.002  # %0.1 → %0.2 buffer (daha esnek fill)
    ...
    def check_watchlist(self, strategy_engine):
        ...
        for item in watching_items:
            ...
            # Zorunlu: direction_ok (5m'de net ters trend → fail)
            # Zorunlu: price_ok (mum yönü doğru olmalı)
            # Esnek: volume_ok esnek (%40) — 5m mumlar genelde düşük hacimli
            # ★ FIX: price_ok zorunlu yapıldı (eskiden price_ok OR body_ok idi)
            # body_ok tek başına yeterli değil — yanlış yöne gövdeli mum onay sayılmamalı
